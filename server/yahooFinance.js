...
async function fetchStockData(symbol, period = '5y') {
  try {
    ...
    // Make the API request with proper headers to avoid CORS issues
    ...
    })).filter((d) => d.close != null); // Filter out null/undefined values
...
async function fetchMultipleStocks(symbols, period = '5y') {
