...
async function calculatePortfolioPerformance(assets) {
  try {
    ...
    for (const symbol of symbols) {
      ...
      for (const dataPoint of stockInfo.data) {
        ...
        if (!dateToDataMap.has(dateStr)) {
          dateToDataMap.set(dateStr, new Map());
        }
    ...
    const stats = calculatePerformanceStats(dates, normalizedValues);
...
function calculatePerformanceStats(dates, values) {
  const periods = [
    { name: '1 Year', years: 1 },
